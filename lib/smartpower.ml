...
let filter raw_data =
  let raw_data = String.split_on_char ',' raw_data |> List.map String.trim in
  let filter_float raw_data =
    let open Re.Str in
    if string_match float_re raw_data 0 then
      matched_string raw_data |> Float.of_string
    else raise (Failure "Not_found")
    (* FIXME: improve error *)
  in
  List.map filter_float raw_data
...
let observe sock =
  ...
  if List.length energy_data <> 4 then raise (Failure "Not_found") (* FIXME *)
  else
